namespace Configastic.SharedModels.Models.Utils
{
    public static class OrionCRC
    {
        ...
        public static bool IsCrcValid(IEnumerable<byte> packetWithCrc)
        {
            var packet = packetWithCrc.ToList();
            var crc = packet.Last();

            packet.RemoveAt(packet.Count - 1);
            var packetArray = packet.ToArray();

            if (GetCrc8(packetArray).First() == crc)
                return true;
