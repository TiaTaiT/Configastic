...
namespace Configastic.SharedModels.Models.BolidDevices
{
    public class Signal20P : OrionDevice, IShleifs, IRelays
    {
        ...
        private List<byte[]> GetBaseConfig()
        {
        ...
        private List<byte[]> GetRelaysProps()
        {
        ...
        private List<byte[]> GetRelaysConfigs<T>(T relay) where T : Relay
        {
        ...
        private IEnumerable<byte[]> GetRelayConfig<T>(T relay, ushort offset) where T : Relay
        {
            ...
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, (byte)relay.OutputType]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)RelayPropertiesOffsets.ControlProgram));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, (byte)relay.ControlProgram]);
        ...
        private static List<byte[]> GetSupervisedRelayConfig(ushort offset, SupervisedRelay supervisedRelay)
        {
            var result = new List<byte[]>();

            byte[] bytes = BitConverter.GetBytes((ushort)(offset + (ushort)SupervisedRelayPropertiesOffsets.ActivatedMonitoring));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, (byte)supervisedRelay.ActivatedOutputType]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)SupervisedRelayPropertiesOffsets.DeactivatedMonitoring));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, (byte)supervisedRelay.DeactivatedOutputType]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)SupervisedRelayPropertiesOffsets.MonitoringModes));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, (byte)supervisedRelay.ActivatedOutputMode]);
            ...
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, tresholdA[0], tresholdA[1]]);
            ...
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, tresholdB[0], tresholdB[1]]);
        ...
        private List<byte[]> GetInputsProps(Func<Shleif, IEnumerable<byte[]>> inputParameters)
        {
            var bytesList = new List<byte[]>();
        ...
        private IEnumerable<byte[]> GetInputProperties(Shleif shleif)
        {
            ...
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.Zone]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ArmingDelay));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ArmingDelay]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.InputAnalysisDelayAfterReset));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.InputAnalysisDelayAfterReset]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ShuntTime));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ShuntTime]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ActivationDelay1));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ActivationDelay1]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ActivationDelay2));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ActivationDelay2]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ActivationDelay3));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ActivationDelay3]);

            bytes = BitConverter.GetBytes((ushort)(offset + (ushort)InputPropertiesOffsets.ActivationDelay4));
            result.Add([(byte)OrionCommands.WriteToDeviceMemoryMap, bytes[0], bytes[1], 0x00, shleif.ActivationDelay4]);
        ...
        private static List<byte[]> GetBitsShleifRelayTune(Shleif shleif, ushort offset)
        {
        ...
        private static List<byte[]> GetBitsShleifTune(Shleif shleif, ushort offset)
        {
        ...
        private static byte[] GetPromoter()
        {
