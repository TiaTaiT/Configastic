...
namespace Configastic.SharedModels.Models.BolidDevices
{
    public class C2000Ethernet : EthernetOrionDevice, ITransaction, IOrionNetTimeouts
    {
        ...
        private static string CidrToString(int cidr)
        {
        ...
        {
            var addr = address;
            var range = (uint)C2000Ethernet.ConvertStringToRange(addr);
            var bitsCounter = 0;
        ...
        private static int ConvertStringToRange(string addrStr)
        {
        ...
        private static byte[] GetEthernetTune(string ip, string netmask, string gateway, string firstDNS, string secondDNS)
        {
        ...
        private static byte[] GetDhcpStatus(bool dhcpEnable)
        {
        ...
        private static byte[] GetTimeout(ushort timeout)
        {
        ...
        private static byte[] GetPause(ushort pause)
        {
        ...
        private static byte[] GetAccessNotifySign(bool accessNotifySign)
        {
        ...
        private static byte[] GetPauseBeforeResponseRs(ushort pause)
        {
            var bytes = BitConverter.GetBytes(pause);
        ...
        private List<byte[]> GetRemoteDevices(IEnumerable<C2000Ethernet> devices)
        {
            if (devices.Count() > 15)
                return [];
            ...
            foreach (var device in devices)
            {
                ...
                cmd.Add(C2000Ethernet.GetRemoteDeviceIp(device.AddressIP, addressOffset));

                cmd.Add(C2000Ethernet.GetRemoteDeviceUdp(device.DestinationUdp, (ushort)(addressOffset + 0x20)));

                cmd.Add(C2000Ethernet.GetRemoteDeviceUdpType((byte)device.UdpPortType, (ushort)(addressOffset + 0x38)));
        ...
        private static List<byte[]> GetRemoteDeviceIpByByte(string sendStr, ushort addressOffset)
        {
            if (IsNullOrEmpty(sendStr))
                return [];
        ...
        private static byte[] GetRemoteDeviceUdp(ushort udp, ushort addressOffset)
        {
            if (udp == 0)
                return [];
        ...
        private static byte[] GetRemoteDeviceUdpType(byte udpType, ushort addressOffset)
        {
        ...
        private static byte[] GetRemoteDeviceCryptoKey(string cryptoKey, ushort addressOffset)
        {
            // stub method
            /*
        ...
        private static byte[] GetMasterSlaveUdp(ushort udp)
        {
            var bytes = BitConverter.GetBytes(udp);
        ...
        private static byte[] GetConfirmationTimeout(byte timeout)
        {
        ...
        private static byte[] GetConnectionTimeout(byte timeout)
        {
            var cmd = new byte[] { 0x41, 0x43, 0x00, 0x00, timeout };
            return cmd;
        }

        private static byte[] GetFreeConnectionTune(C2000Ethernet.UdpType udpType, bool allowFreeConnection)
        {
        ...
        private static byte[] GetFreeConnectionUdp(ushort udp)
        {
            var bytes = BitConverter.GetBytes(udp);
        ...
        private static byte[] GetTransparentTune(ushort udp, C2000Ethernet.TransparentProtocolType protocol, bool crypto)
        {
        ...
        private static byte[] ReadDeviceNetworkSettings()
        {
            return [0x43, 0x00, 0x00, 0x00, 0x40]; // 7f 08 6e 43 00 00 00 40
        }

        private static byte[] GetSecondPowerControl()
        {
